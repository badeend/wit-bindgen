...
fn classify_constructor_return_type(
    ...
    fn classify(
        ...
        match &typedef.kind {
            TypeDefKind::Handle(Handle::Own(id)) if *id == resource_id => {
    ...
    classify(resolve, resource_id, result)
        .expect("constructor must return either an owned handle to itself or a result with an owned handle to itself")
}
